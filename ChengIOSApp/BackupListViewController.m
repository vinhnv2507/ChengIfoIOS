...
static NSString *CIResultText(NSDictionary *meta, NSError *error, NSString *fallbackOK) {
    ...
    if (meta[@"keychainItems"]) {
        ...
        if ([meta[@"includeAppData"] boolValue] && [meta[@"keychainItems"] unsignedIntegerValue] == 0) {
            [text appendString:@"\nCanh bao: Keychain 0. Cai ldid (Apps Manager ldid / Procursus). Can Keychain N>0, withData N>0, kcUid 501. Cai 1.2.54, cai ldid, Respring, roi tao backup MOI khi app dang login. Backup cu khong co Caches/tmp/companion co the khong giu login."];
        } else if ([meta[@"includeAppData"] boolValue] && [meta[@"keychainWithData"] unsignedIntegerValue] == 0) {
            [text appendString:@"\nCanh bao: Keychain khong co data. Restore se mat login. Cai ldid, Respring va backup lai khi dang login bang 1.2.54."];
        }
    }
    if (meta[@"asRoot"]) {
...
void ChengIOSRunRestore(UIViewController *host, NSString *backupID, BOOL restoreProfile, BOOL restoreAppData, BOOL silent) {
    ...
    void (^go)(void) = ^{
        CIRunBusy(host, @"Dang restore", ^(void (^done)(NSString *, NSString *)) {
            ...
            NSString *msg = error ? ChengIOSBackupErrorMessage(error) : (ok ? @"Da restore sandbox + keychain. Force-quit Facebook/Shopee/TikTok roi mo lai. Facebook/TikTok can backup MOI bang 1.2.54 khi dang login; backup cu khong du Caches/tmp/companion. Can Data>0, Keychain withData>0, kcUid 501." : @"Restore that bai.");
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if ([item[@"includeAppData"] boolValue] && [bundles isKindOfClass:[NSArray class]]) {
        [detail appendFormat:@"  Â·  %lu app  Â·  %@", (unsigned long)bundles.count, CIBytesString([item[@"bytes"] unsignedLongLongValue])];
        if (item[@"keychainItems"]) {
            [detail appendFormat:@"  Â·  KC %@", item[@"keychainItems"]];
        }
        if (item[@"kcUid"]) {
            [detail appendFormat:@"  Â·  kcUid %@", item[@"kcUid"]];
        }
        if (item[@"asRoot"]) {
            [detail appendFormat:@"  Â·  root %@", [item[@"asRoot"] boolValue] ? @"CO" : @"KHONG"];
        }
    } else {
        [detail appendString:@"  Â·  ho so"];
    }
